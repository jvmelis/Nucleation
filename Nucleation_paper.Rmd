...
mod_surv2 <- lme4::glmer(cbind(n, 24 - n) ~ bloco +(1|bloco), data = survival_planted2024, family = binomial(link = "logit"))
mod_surv3 <- lme4::glmer(cbind(n, 24 - n) ~ bloco+ +(1|bloco), data = survival_planted2024, family = binomial(link = "logit"))
...
Planted trees in treatment `F25` have `r round((1 - exp(-0.224))*100,2)`% *less* likely to survive (`OR=exp(-0.224)`) than in treatment `100%` ($p<0.1$)
...
```{r}
sampled_species <-readxl::read_xlsx("sampled_species.xlsx")
sampled_species |> 
