...
# After first submission
  ...
  * Inclusion of species diversity of recruiting plants: item [I.D](#d-species-richness-of-recruits), with rarefaction and Proportion of Biotic/Abiotic Seed Dispersal and  Planted/Unplanted species.
...
pkg_list <-c("tidyverse", "lme4", "DHARMa", "emmeans", "multcomp", "iNEXT",
             "car", "performance", "cowplot", "MuMIn", "WorldFlora")
...
#### Table 1 - List of sampled species (planted and recruits)
...
emmeans::emmeans(mod_surv1, ~ tratamento) |> 
...
```{r}
recruitment <- readxl::read_xlsx(paste0(path, "/Itatinga_Nuc_recruitment.xlsx"))
```

Visual exploration of small recruitment ( *saplings* )
...
recruitment |>
  ...
  facet_grid(plantio~tratamento)+
  theme_bw()
```

-   Only 2024 and by block

```{r}
small_regenerants_2024 <- recruitment |> 
  dplyr::filter(ano==2024) |>
  #dplyr::filter(tratamento!="100") |>
  dplyr::mutate(
    n = dplyr::if_else(is.na(n_small), 0, n_small))
  # dplyr::group_by(tratamento, plantio, bloco) |> dplyr::summarise(n = sum(n_small, na.rm = TRUE), .groups = "drop")
```



### (ii) Analysis for smaller recruits

```{r}
modelo_reg1 <- glmmTMB::glmmTMB(n ~ tratamento*plantio + (1 | bloco),
                   # family = poisson(),
                   family = glmmTMB::nbinom2(),
                   data = small_regenerants_2024)

modelo_reg2 <-  glmmTMB::glmmTMB(n ~ tratamento + (1 | bloco),
                   #family = poisson(),
                   family = glmmTMB::nbinom2(),
                   data = small_regenerants_2024)
modelo_reg3 <-  glmmTMB::glmmTMB(n ~ plantio + (1 | bloco),
                   # family = poisson(),
                   family = glmmTMB::nbinom2(),
                   data = small_regenerants_2024)
modelo_reg_null <-  glmmTMB::glmmTMB(n ~ 0 + (1 | bloco),
                   # family = poisson(), # Poisson
                   #family = glmmTMB::nbinom2(), # ZINB
                   data = small_regenerants_2024)
# NBinom 1738.191 (null) |	1687.567 (plantio*tratamento) | 1715.110	(only tratamento) | 1692.276	(only plantio)
# Poisson  3744.473	|	3164.141		|	3604.090		|	3431.868
...
If a model is "singular", this means that some dimensions of the variance-covariance matrix have been estimated as exactly zero. This often occurs for mixed models with complex random effects structures. Therefore, there is a model convergence problem, and we have to change the random effects.
...
```{r}
emmeans::emmeans(modelo_reg1, ~ tratamento|plantio) |> 
  multcomp::cld(details = TRUE)
```
...
```{r}
emmeans::emmeans(modelo_reg2, ~ tratamento) |> 
  multcomp::cld(details = TRUE)
```
...
```{r}
emmeans::emmeans(modelo_reg1, ~ plantio|tratamento) |> 
  multcomp::cld(details = TRUE)
```
...
```{r}
sjPlot::tab_model(modelo_reg1)
m_fix <- update(modelo_reg1, . ~ . - (1|bloco))

performance::check_singularity(modelo_reg1)
anova(modelo_reg1, m_fix)   # se não há ganho, var(bloco) pode ser ~0
broom.mixed::tidy(modelo_reg1) |> dplyr::mutate(dplyr::across(dplyr::where(is.numeric), ~ round(.x, 3))) |> knitr::kable()
```



### (iii) Analysis for larger recruitment


```{r}
large_regenerants_2024 <- recruitment |> 
  dplyr::filter(ano==2024) |>
  # dplyr::filter(tratamento!="100") |>
  dplyr::mutate(n = dplyr::if_else(is.na(n_large), 0, n_large))
large_regenerants_2024 |>
  dplyr::group_by(tratamento, plantio, bloco) |> 
  #dplyr::summarise(n = sum(n, na.rm = TRUE), .groups = "drop") |>
  ggplot(aes(x=plantio, y=n, color=bloco))+
  geom_point()+facet_wrap(~tratamento)+theme_bw()
...
# ZINB 878.1779		|	862.1359		 |	861.3872	|	856.8599
# ZIP  1048.272   | 1006.612		 |	1016.957	|	1015.703
# Hurdle 873.7383	| 865.1757		|	861.7863		|	965.0392
...
0.032/ 0.037 variance explained by fixed effects and by the entire model (around `r 0.03253385/0.03665597` % is explained by fixed effects)
...
### (iv) Table S2 - Recruitment

`pioneer`: colonizers, common in pastures, open areas.

Only smaller recruits (C30 < 5 cm and h>50cm)

```{r}
recruitment_pioneer <- readxl::read_xlsx(paste0(path, "/Itatinga_Nuc_recruitment_pioneers.xlsx"))

recruitment_pioneer |>
  dplyr::mutate(n_small = dplyr::if_else(is.na(n_small), 0, n_small))|>
  group_by(tratamento, bloco, plantio) |>
  dplyr::summarise(
    total_n_small = sum(n_small, na.rm = TRUE),
    n_small_pioneiras = sum(n_small[pioneer == TRUE], na.rm = TRUE),
    prop_pioneiras = n_small_pioneiras / total_n_small,
    .groups = "drop"
  ) |>
  #dplyr::group_by(tratamento,plantio)|>
  dplyr::summarise(mean_prop_pioneiras = mean(prop_pioneiras),
                   se_prop_pioneiras = sd(prop_pioneiras)/sqrt(dplyr::n()))
```

Larger recruits (C30 > 5cm):

```{r}
recruitment_pioneer |>
  dplyr::mutate(n_large = dplyr::if_else(is.na(n_large), 0, n_large))|>
  group_by(tratamento, plantio, bloco) |>
  dplyr::summarise(
    total_n = sum(n_large, na.rm = TRUE),
    n_pioneiras = sum(n_large[pioneer == TRUE], na.rm = TRUE),
    prop_pioneiras = n_pioneiras / total_n,
    .groups = "drop"
  ) |>
  #dplyr::group_by(tratamento,plantio)|>
  dplyr::summarise(mean_prop_pioneiras = mean(prop_pioneiras),
                   se_prop_pioneiras = sd(prop_pioneiras)/sqrt(dplyr::n()),
                   min_prop_pioneiras = min(prop_pioneiras),
                   max_prop_pioneiras = max(prop_pioneiras))
```
...
recruitment_pioneer |>
  dplyr::mutate(
    n_large = dplyr::if_else(is.na(n_large), 0, n_large),
    n_small = dplyr::if_else(is.na(n_small), 0, n_small),
    n_both = n_small + n_large)|>
  group_by(tratamento, plantio, bloco) |>
  dplyr::summarise(
    total_n = sum(n_both, na.rm = TRUE),
    n_pioneiras = sum(n_both[pioneer == TRUE], na.rm = TRUE),
    prop_pioneiras = n_pioneiras / total_n,
    .groups = "drop"
  ) |>
  #dplyr::group_by(tratamento,plantio)|>
  dplyr::summarise(mean(prop_pioneiras),
                   sd(prop_pioneiras)/sqrt(dplyr::n()),
                   min(prop_pioneiras),
                   max(prop_pioneiras))
```
...
```{r}
density_recruits_n_m2 <- recruitment_pioneer |> 
  dplyr::filter(ano == 2024) |>
  dplyr::mutate(
    n_small = dplyr::if_else(is.na(n_small), 0, n_small),
    n_large = dplyr::if_else(is.na(n_large), 0, n_large)) |>
  dplyr::group_by(tratamento, bloco, plantio) |>
  dplyr::summarise(
...
prop_table <- tidyr::tibble(
  tratamento = unique(recruitment_pioneer$tratamento),
  prop_CP = c(0.25, 0.25, 0.5, 0.5, 1),
  prop_SP = 1 - prop_CP
)


result_regenerant <- dens_CP_SP |>
  dplyr::left_join(prop_table, by = "tratamento") %>%
  dplyr::mutate(
    n_small_total_ha = dplyr::coalesce(n_small_ha_CP * prop_CP, 0)  + dplyr::coalesce(n_small_ha_SP * prop_SP, 0),
    n_large_total_ha = dplyr::coalesce(n_large_ha_CP * prop_CP, 0) + dplyr::coalesce(n_large_ha_SP * prop_SP, 0)
  ) |>
...
result_regenerant |>
  dplyr::group_by(tratamento)|>
  dplyr::summarise(
    mean = round(mean(n_small_ha_CP, na.rm=TRUE),1),
    min = round(min(n_small_ha_CP, na.rm=TRUE),0),
    max = round(max(n_small_ha_CP, na.rm=TRUE),0)
  )
```
...
result_regenerant |>
  dplyr::group_by(tratamento)|>
  dplyr::summarise(
    mean = round(mean(n_small_ha_SP, na.rm=TRUE),1),
    min = round(min(n_small_ha_SP, na.rm=TRUE),0),
    max = round(max(n_small_ha_SP, na.rm=TRUE),0)
  )
```
...
Inside planted areas:

```{r}
result_regenerant |>
  dplyr::group_by(tratamento)|>
  dplyr::summarise(
    mean = round(mean(n_large_ha_CP, na.rm=TRUE),1),
    min = round(min(n_large_ha_CP, na.rm=TRUE),0),
    max = round(max(n_large_ha_CP, na.rm=TRUE),0)
  )
```


Outside planted areas:

```{r}
result_regenerant |>
  dplyr::group_by(tratamento)|>
  dplyr::summarise(
    mean = round(mean(n_large_ha_SP, na.rm=TRUE),1),
    min = round(min(n_large_ha_SP, na.rm=TRUE),0),
    max = round(max(n_large_ha_SP, na.rm=TRUE),0)
  )
```
...
```{r}
data_fig_small <- result_regenerant |>
  dplyr::group_by(tratamento)|>
  dplyr::summarise(
    mean_inside = round(mean(n_small_ha_CP, na.rm=TRUE), 1),
    se_inside = round((sd(n_small_ha_CP, na.rm=TRUE))/sqrt(dplyr::n()), 1),
    mean_outside = round(mean(n_small_ha_SP, na.rm=TRUE), 1),
    se_outside = round((sd(n_small_ha_SP, na.rm=TRUE))/sqrt(dplyr::n()), 1)
  ) |>
   tidyr::pivot_longer(cols = -tratamento, names_to = c(".value", "side"), names_sep = "_") |> 
  dplyr::mutate(
...
letras <- data.frame(
  tratamento = rep(levels(as.factor(data_fig_small$tratamento)), each=2),
  side = levels(as.factor(data_fig_small$side)),
  label = c("AB     ","      a",
            "AB     ","      a",
            "B      ","      a",
            "B      ","      a",
...
fig_small <- data_fig_small |>
  ...
  scale_y_continuous(expand =c(0,0), limits = c(0,7900), breaks = seq(0,  7500, by=1500))+
  labs(fill = "", 
...
data_fig_large <-result_regenerant |>
  dplyr::group_by(tratamento)|>
  dplyr::summarise(
    mean_inside = round(mean(n_large_ha_CP, na.rm=TRUE), 1),
    se_inside = round((sd(n_large_ha_CP, na.rm=TRUE))/sqrt(dplyr::n()), 1),
    mean_outside = round(mean(n_large_ha_SP, na.rm=TRUE), 1),
    se_outside = round((sd(n_large_ha_SP, na.rm=TRUE))/sqrt(dplyr::n()), 1)
  ) |>
   tidyr::pivot_longer(cols = -tratamento, names_to = c(".value", "side"), names_sep = "_") |> 
  dplyr::mutate(
    tratamento = ordered(tratamento,levels = ordem,
...
fig_large <- data_fig_large |>
  ...
  theme(
    ...
    legend.title = element_text(size = 8))
fig_large
# ggsave("fig_large_recruits.png", plot=fig_large, width=90, height = 60 ,unit="mm")
```
...
fig_recruits <- cowplot::plot_grid(#fig_grass+theme(axis.text.x=element_blank()), 
                                   fig_small + theme(axis.text.x=element_blank()), 
                                   fig_large, 
                                   ncol = 1,
                                   labels = c("(a)", "(b)"), label_size = 10, label_x = 0.01, label_y = 1)
fig_recruits
...
```{r}
planted_species <- readxl::read_xlsx("Itatinga_Species_nucleation.xlsx", sheet = "planted species") |>
  dplyr::rename(scientificName = Species)
planted_species |> knitr::kable(format = "html")
```
...
## D) Species Richness of recruits
...
dados_list <- species_table |>
  ...
  dplyr::summarise(n_total = sum(n_total), .groups = "drop") |>
...
species_table|>
  dplyr::arrange(Treatment, Block, Plot)
```
...
prop_regenerant_full <- species_table |>
  dplyr::mutate(
    ...
    biotic_fruit = ifelse(`Seed dispersal`=="biotic", "Yes", "No"))|>
  dplyr::group_by(Treatment, Planted, Block, Plot) |>
  dplyr::summarise(
    n_species = dplyr::n(),
    n_ind = sum(n_total),
    n_planted_trees = sum(ifelse(planted_tree == "Yes", n_total, 0), na.rm = TRUE),
    n_planted_spp = sum(planted_tree == "Yes"), 
    n_biotic_trees  = sum(ifelse(biotic_fruit == "Yes", n_total, 0), na.rm = TRUE),
    n_biotic_spp  = sum(biotic_fruit == "Yes"),
    n_biotic_spp = ifelse(is.na(n_biotic_spp), 0, n_biotic_spp),
    n_biotic_trees = ifelse(is.na(n_biotic_trees), 0, n_biotic_trees),
    n_planted_spp = ifelse(is.na(n_planted_spp), 0, n_planted_spp),
    n_planted_trees = ifelse(is.na(n_planted_trees), 0, n_planted_trees),
    .groups = "drop") |>
  dplyr::mutate(
    prop_planted = n_planted_trees / n_ind,
    prop_planted_spp = n_planted_spp / n_species,
    
    prop_biotic  = n_biotic_trees  / n_ind,
    prop_biotic_spp  = n_biotic_spp  / n_species
  )
```
...
prop_regenerant_full |>
  dplyr::group_by(Treatment, Planted)|>
  dplyr::summarise(
    prop_planted_mean = mean(prop_planted, na.rm=T),
    prop_planted_min = min(prop_planted, na.rm=T),
    prop_planted_max = max(prop_planted, na.rm=T),
    ...
    prop_biotic_mean = mean(prop_biotic, na.rm=T),
    prop_biotic_min = min(prop_biotic, na.rm=T),
    prop_biotic_max = max(prop_biotic, na.rm=T),
    
    prop_biotic_spp_mean = mean(prop_biotic_spp, na.rm=T),
    prop_biotic_spp_min = min(prop_biotic_spp, na.rm=T),
    prop_biotic_spp_max = max(prop_biotic_spp, na.rm=T)
  ) |>
  dplyr::mutate(
    prop_biotic_text = sprintf("%.1f (%.1f - %.1f)", 
                                round(prop_biotic_mean,3)*100, 
                                round(prop_biotic_min,3)*100, 
                                round(prop_biotic_max,3)*100),
    prop_biotic_spp_text = sprintf("%.1f (%.1f - %.1f)", 
                                    round(prop_biotic_spp_mean,3)*100, 
                                    round(prop_biotic_spp_min,3)*100, 
                                    round(prop_biotic_spp_max,3)*100),
    prop_planted_text = sprintf("%.1f (%.1f - %.1f)", 
                                round(prop_planted_mean,3)*100, 
                                round(prop_planted_min,3)*100, 
                                round(prop_planted_max,3)*100),
    prop_planted_spp_text = sprintf("%.1f (%.1f - %.1f)", 
                                    round(prop_planted_spp_mean,3)*100, 
                                    round(prop_planted_spp_min,3)*100, 
                                    round(prop_planted_spp_max,3)*100)
    ) |>
  dplyr::select(Treatment, Planted, prop_biotic_text, prop_biotic_spp_text, prop_planted_text, prop_planted_spp_text) |>
  tidyr::pivot_wider(
    names_from = Treatment,
    values_from = c(prop_biotic_text, prop_biotic_spp_text, prop_planted_text, prop_planted_spp_text)
    ) # |> writexl::write_xlsx("resulta_prop_recruit.xlsx")
```



#### (ii) Statistical analysis for the proportions


**Propotion of individuals with biotic seed dispersal**

```{r}
mod_binomial <-lme4::glmer(
  cbind(n_biotic_trees, n_ind - n_biotic_trees) ~ Treatment*Planted + (1 | Block),
  family = binomial(link = "logit"),
  data = prop_regenerant_full
)
performance::check_overdispersion(mod_binomial)
DHARMa::plotResiduals(mod_binomial)
summary(mod_binomial)
emmeans::emmeans(mod_binomial, pairwise ~ Treatment | Planted) |> multcomp::cld(adjust="sidak")
emmeans::emmeans(mod_binomial, pairwise ~  Planted | Treatment) |> multcomp::cld(adjust="sidak")
```


```{r}
broom.mixed::tidy(mod_binomial) |> dplyr::mutate(dplyr::across(dplyr::where(is.numeric), ~ round(.x, 3)))
sjPlot::tab_model(mod_binomial)
```


**Propotion of species with biotic seed dispersal**

```{r}
mod_binomial <-lme4::glmer(
  cbind(n_biotic_spp, n_species - n_biotic_spp) ~ Treatment*Planted + (1 | Block),
  family = binomial(link = "logit"),
  data = prop_regenerant_full
)
performance::check_overdispersion(mod_binomial)
...
Overdispersion, then, Beta Binomial

```{r}
mod_betabinomial<-glmmTMB::glmmTMB(
  cbind(n_biotic_spp, n_species - n_biotic_spp) ~ Treatment*Planted + (1 | Block),
  family = glmmTMB::betabinomial(),
  data = prop_regenerant_full
)
performance::check_overdispersion(mod_betabinomial)

DHARMa::plotResiduals(mod_betabinomial)
summary(mod_betabinomial)
emmeans::emmeans(mod_betabinomial, pairwise ~ Treatment | Planted) |> multcomp::cld(adjust="sidak")
emmeans::emmeans(mod_betabinomial, pairwise ~  Planted | Treatment) |> multcomp::cld(adjust="sidak")
```


```{r}
broom.mixed::tidy(mod_betabinomial) |> dplyr::mutate(dplyr::across(dplyr::where(is.numeric), ~ round(.x, 3)))
sjPlot::tab_model(mod_betabinomial)
```

**Propotion of individuals from planted species**

```{r}
mod_binomial <-lme4::glmer(
  cbind(n_planted_trees, n_ind - n_planted_trees) ~ Treatment*Planted + (1 | Block),
  family = binomial(link = "logit"),
  data = prop_regenerant_full
)
performance::check_overdispersion(mod_binomial) # heterokedasticity
DHARMa::plotResiduals(mod_binomial)
summary(mod_binomial)
emmeans::emmeans(mod_binomial, pairwise ~ Treatment | Planted) |> multcomp::cld(adjust="sidak")
emmeans::emmeans(mod_binomial, pairwise ~  Planted | Treatment) |> multcomp::cld(adjust="sidak")
...
Summary of these proportions:

```{r}
prop_regenerant <-prop_regenerant_full |>
  dplyr::group_by(Treatment, Planted) |>
  dplyr::summarise(
    prop_planted_mean = mean(prop_planted, na.rm = TRUE),
    prop_planted_se   = sd(prop_planted, na.rm = TRUE) / sqrt(dplyr::n()),
    
    prop_spp_planted_mean = mean(prop_planted_spp, na.rm = TRUE),
    prop_spp_planted_se   = sd(prop_planted_spp, na.rm = TRUE) / sqrt(dplyr::n()),
    
    prop_biotic_mean  = mean(prop_biotic, na.rm = TRUE),
    prop_biotic_se    = sd(prop_biotic, na.rm = TRUE) / sqrt(dplyr::n()),
    
    prop_spp_biotic_mean  = mean(prop_biotic_spp, na.rm = TRUE),
    prop_spp_biotic_se    = sd(prop_biotic_spp, na.rm = TRUE) / sqrt(dplyr::n()),
    .groups = "drop"
  ) |>
  tidyr::separate(Treatment, into = c("Treatment", "Percent"), sep = "_")|>
  dplyr::mutate(
    Percent = ifelse(Percent=="Planted", "100", Percent),
    Percent = ordered(Percent, levels = c("25", "50", "100"))
  ) 
  
summary_species <- estimativas |>
  dplyr::filter(Method=="Observed")|>
  dplyr::mutate(
    Percent = ifelse(Percent=="Planted", "100", Percent),
    Percent = ordered(Percent, levels = c("25", "50", "100"))
  ) |>
  dplyr::arrange(Order.q,Percent) |> 
  dplyr::select(Treatment:qD.UCL)|> 
  dplyr::mutate(dplyr::across(dplyr::where(is.numeric), ~ round(.x, 2))) |> unique()|>
  dplyr::right_join(prop_regenerant, by=c("Treatment", "Percent", "Planted"))

# summary_species |> writexl::write_xlsx("resulta_diversidade_nuc.xlsx")
```


#### (iii) Figure Recruits' species richness
...
curvas_df0 <- estimativas |> 
  ...
  dplyr::mutate(
    ...
    Treatment = ifelse(Treatment=="Full", "Total area", Treatment)
  ) 
```
...
fig_recruit_rich
# ggsave("fig_richness_recruit.png", plot=fig_recruit_rich, width=90, height = 210 ,unit="mm")
```
