...
pkg_list <-c("tidyverse", "lme4", "DHARMa", "emmeans", "multcomp", "car", "performance", "cowplot", "MuMIn")
...
## A) Survival of planted trees - ok
...
```{r}
mod_surv1 <- lme4::glmer(cbind(n, 24 - n) ~ tratamento +(1|bloco/parcela), data = survival_planted2024, family = binomial(link = "logit"))
mod_surv2 <- lme4::glmer(cbind(n, 24 - n) ~ bloco +(1|bloco/parcela), data = survival_planted2024, family = binomial(link = "logit"))
mod_surv3 <- lme4::glmer(cbind(n, 24 - n) ~ bloco+ +(1|bloco/parcela), data = survival_planted2024, family = binomial(link = "logit"))
mod_surv_null <-lme4::glmer(cbind(n, 24 - n) ~ 0 +(1|bloco/parcela), data=survival_planted2024, family = binomial(link = "logit"))
AIC(mod_surv1, mod_surv2, mod_surv3, mod_surv_null)
```
...
Which it is `r round(0.117/0.544, 3)`
...
Planted trees in treatment `N25` have `r  round((1 - exp(-0.548))*100,2)`% *less* likely to survive (`OR=exp(-0.548)`) than in treatment `100%` ($p<0.001$)

```{r}
round((1 - exp(-0.548))*100,2)
...
modelo_gc24b <-  glmmTMB::glmmTMB(grass_perc ~ tratamento+plantio + (1 | bloco),
                   ziformula = ~ 1, family = glmmTMB::beta_family(), data=grass_cover2024)
modelo_gc24c <-  glmmTMB::glmmTMB(grass_perc ~ plantio + (1 | bloco), 
                                  family = glmmTMB::beta_family(), data=grass_cover2024)
modelo_gc24d <-  glmmTMB::glmmTMB(grass_perc ~ tratamento + (1 | bloco), 
                                  family = glmmTMB::beta_family(), data=grass_cover2024)
modelo_gc24null <-  glmmTMB::glmmTMB(grass_perc ~ 0 + (1 | bloco), 
                                     family = glmmTMB::beta_family(), data=grass_cover2024)
...
#### List of recruitment species

```{r}
table_recruits <- trees_clean |>
  dplyr::filter(RP!="planted") |>
  dplyr::filter(ano == 2024) |>
  dplyr::mutate(
    ...
    dplyr::group_by(tratamento, family, scientificName)|>
  dplyr::summarise(n_large = dplyr::n(), .groups = "drop") |>
  tidyr::pivot_wider(names_from = tratamento, values_from = n_large, values_fill = 0) |> 
  dplyr::arrange(family, scientificName)
table_recruits |> knitr::kable(format = "html")
table_recruits  |> writexl::write_xlsx("Table Recruits.xlsx")
```
...
modelo_reg_null <-  glmmTMB::glmmTMB(n ~ 0 + (1 | bloco),
                   ...
                   data = regenerants_2024)
# ZIP  3746.473 (null) | 3166.141 (plantio*tratamento)  3606.090 (só tratamento) | 3433.868 (só plantio)
# ZINB  1740.191 (null) | 1689.567 (plantio*tratamento)  1717.110 (só tratamento) | 1694.276 (só plantio)
# Hurdle  1740.185 (null) | 1689.565 (plantio*tratamento)  1717.106 (só tratamento) | 1694.273 (só plantio)
...
modelo_reg_null <-  glmmTMB::glmmTMB(n ~ 0 + (1 | bloco),
                   ziformula = ~1,
                    ...
                    # family = glmmTMB::truncated_nbinom2(), # hurdle
                   data = regenerants_2024)
# ZIP   920 (só plantio)   plantio*tratamento
# ZINB  770  (só plantio)  plantio*tratamento
# Hurdle 772  (só plantio)   plantio*tratamento
...
0.037/ 0.048
#MuMIn::r.squaredGLMM(modelo_reg1)
#0.088/ 0.095
```

0.037/ 0.048 variance explained by fixed effects and by the entire model (around `r 0.037/ 0.048` % is explained by fixed effects)
...
Only smaller recruits (0.5 m ≤  height ≤ 2 m)
...
```{r}
par(mfrow=c(1,2))
DHARMa::plotResiduals(AGB_model6)
DHARMa::plotQQunif(AGB_model6)
```
...
```{r}
emmeans::emtrends(AGB_model6, pairwise ~ tratamento, var = "Ano", poly.trend = TRUE) |> multcomp::cld()
```
...
```{r}
emmeans::emtrends(AGB_inside_m6, pairwise ~ tratamento, var = "Ano", poly.trend = TRUE) |> multcomp::cld()
```
...
A tendência da variação em torno da média de CHM foi a mesma ao longo do tempo entre os tratamentos (n.s.). As diferenças significativas ($p<0.001$) observadas entre o tratamento 100% planting em relação com os outros tratamentos em 2022 manteve-se em 2024.
...
lidar_wide |>
  ...
  dplyr::summarise(Mean = round(mean(CHM_mean), 2),
                   SD = round(sd(CHM_mean), 2)) |>
  dplyr::mutate(values = paste(Mean, "pm", SD)) |>
...
lidar_wide |>
  dplyr::group_by(Tratamento)|>
  dplyr::summarise(
    variacao_media = round(mean((CHM_mean[ano == 2024] - CHM_mean[ano == 2022])/2),2),
    desvio_padrao = round(sd((CHM_mean[ano == 2024] - CHM_mean[ano == 2022])/2), 2)
  ) |>
  dplyr::mutate(values = paste(variacao_media, "pm", desvio_padrao)) |>
...
lidar_wide |>
  dplyr::group_by(Tratamento)|>
  dplyr::summarise(
    variacao_media = round(mean((Canopy_cover[ano == 2024] - Canopy_cover[ano == 2022])/2),2),
    desvio_padrao = round(sd((Canopy_cover[ano == 2024] - Canopy_cover[ano == 2022])/2), 2),
    .groups = "drop"
  ) |>
  dplyr::mutate(values = paste(variacao_media, "pm", desvio_padrao)) |>
  dplyr::select(Tratamento, values) 
...
lidar_wide |>
  dplyr::group_by(Tratamento)|>
  dplyr::summarise(
    variacao_media = round(mean((Rugosity_SD[ano == 2024] - Rugosity_SD[ano == 2022])/2),2),
    erro_padrao = round(sd((Rugosity_SD[ano == 2024] - Rugosity_SD[ano == 2022])/2), 2),
                   .groups="drop") |>
  dplyr::mutate(values = paste(variacao_media, "pm", erro_padrao)) |>
  dplyr::select(Tratamento, values) 
```
...
lidar_wide |>
  dplyr::group_by(Tratamento)|>
  dplyr::summarise(
    variacao_media = round(mean((LAI_mean[ano == 2024] - LAI_mean[ano == 2022])/2),2),
    desvio_padrao = round(sd((LAI_mean[ano == 2024] - LAI_mean[ano == 2022])/2),2),
    .groups = "drop"
  ) |>
  dplyr::mutate(values = paste(variacao_media, "pm", desvio_padrao)) |>
  dplyr::select(Tratamento, values) |>
...
p1<- lidar_wide |>
  ...
  dplyr::summarise(Mean = mean(CHM_mean),
                   ...
                   SE = SD/dplyr::n(),
                   .groups="drop") |>
  ggplot( aes(x = Tratamento, y = Mean, fill = factor(ano))) +
  geom_bar(stat = "identity", position = position_dodge(width = 1), size=1, color="black", linewidth=0.5) +
  geom_errorbar(aes(ymin = Mean - SE, ymax = Mean + SE),
                width = .2, position = position_dodge(width = 1), linewidth=0.5) +
  scale_fill_manual(
...
p2<- lidar_wide |>
  ...
  scale_fill_manual(
    labels = c('5 years', '7 years'), 
    values = c("lightblue", "darkblue")) +
  geom_text(data = letras, aes(x = Tratamento, y = y, label = label), color="black", vjust = -0.5, size=3.5) +
    geom_text(data = porcentos, aes(x = Tratamento, y = y, label = label),size = 3, color="black", vjust = -0.5) +
  labs(color = "", 
       y =  "Canopy cover (%)",
       x = "") +
...
p3<-lidar_wide |>
  ...
  dplyr::mutate(
   ...
   ggplot(aes(x = Tratamento, y = Mean, fill = factor(ano))) +
  geom_bar(stat = "identity", position = position_dodge(width = 1), size=1, linewidth=0.5, color="black") +
  geom_errorbar(aes(ymin = Mean - SE, ymax = Mean + SE),
                width = .2, position = position_dodge(width = 1), linewidth=0.5) +
  scale_fill_manual(
    labels = c('5 years', '7 years'), 
...
# 90 mm x 180 mm
fig_lidar <- cowplot::plot_grid(p1,p2,p3, ncol = 1,
...
```{r}
modeloCC <- lme4::lmer(log(cc_cost) ~ tratamento*ano + (1|bloco), data=cost_raw ) 
par(mfrow=c(1,2))
DHARMa::plotResiduals(modeloCC)
DHARMa::plotQQunif(modeloCC)
emmeans::emmeans(modeloCC, pairwise ~ tratamento | ano) |> multcomp::cld()
...
```{r}
modeloAGB <- lme4::lmer(log(agb_cost) ~ tratamento*ano + (1|bloco), data=cost_raw ) 
par(mfrow=c(1,2))
DHARMa::plotResiduals(modeloAGB)
DHARMa::plotQQunif(modeloAGB)
emmeans::emmeans(modeloAGB, pairwise ~ tratamento | ano) |> multcomp::cld()
```
...
```{r}
emmeans::emtrends(modeloAGB, pairwise ~ tratamento, var = "ano", poly.trend = TRUE)|> multcomp::cld()
```
...
fig_cost1 <- fig_data_cost_raw |>
   ggplot( aes(x = tratamento, y = cost_cc, fill = factor(ano))) +
  geom_bar(stat = "identity", position = position_dodge(width = 1), size=1, linewidth=0.5, color="black") +
  geom_errorbar(aes(ymin = cost_cc - SE_cc, ymax = cost_cc + SE_cc),
                width = .2, position = position_dodge(width = 1), linewidth=0.5) +
  scale_fill_manual(
    labels = c('5 years', '7 years'), 
    values = c("lightblue", "darkblue")) +
  ...
  geom_text(data = letras, aes(x = tratamento, y = y, label = label),size = 4, color="black", vjust = -0.5) +
  theme_classic()+
  theme(
    #panel.grid.major.y = element_blank(),
    #panel.grid.major.x = element_blank(),
...
fig_cost2 <- fig_data_cost_raw |>
  ggplot( aes(x = tratamento, y = cost_agb, fill = factor(ano))) +
  geom_bar(stat = "identity", position = position_dodge(width = 1), size=1, linewidth=0.5, color="black") +
...
fig_cost <- cowplot::plot_grid(fig_cost1, fig_cost2, ncol = 1,
                               labels = c("(a)", "(b)"), label_size = 8, label_x = 0.05, label_y = c(1,1.1))
fig_cost
