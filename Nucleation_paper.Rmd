...
# After first submission
  ...
  * To discuss (e-meeting between authors):
  
    - Change in community composition from time 0 to times 5 and 7, to explore whether some tree species tend to become dominant or whether the species composition in different treatments drift in different directions.
...
curvas_df <- res_inext$iNextEst |>
  dplyr::bind_rows()|>
  tidyr::separate(Assemblage, into = c("Treatment", "Percent", "Planted"), sep = "_")

curvas_df0 <-curvas_df|> dplyr::filter(Order.q==0)|>
  dplyr::mutate(
    Percent = ifelse(Percent=="Planted", "100", Percent),
    Percent = ordered(Percent, levels = c("25", "50", "100")),
...
fig_recruit_rich<-curvas_df0 |>
  ...
  facet_grid(forcats::fct_rev(Percent)~., 
  ...
  labs(
    #color = "Planting schema",
    #linetype = "Planted/Unplanted Area",
    #fill = "Planting schema",
...
```{r, fig.height=8.26, fig.width=3.54}
curvas_df1 <-curvas_df|> dplyr::filter(Order.q==1)|>
  dplyr::mutate(
    Percent = ifelse(Percent=="Planted", "100", Percent),
    Percent = ordered(Percent, levels = c("25", "50", "100")),
    Planted = ifelse(Planted=="Inside", "Planted", "Unplanted"),
...
fig_recruit_Hill1<-curvas_df1 |>
  ggplot(aes(x = m, y = qD, color = Treatment, linetype = Planted)) +
  geom_line(linewidth = 1) +
  geom_ribbon(aes(ymin = qD.LCL, ymax = qD.UCL, fill = Treatment), alpha = 0.2, colour = NA) +
   geom_point(
    data = dplyr::filter(curvas_df1, Method == "Observed"),
    size = 2.5
  ) +
  facet_grid(forcats::fct_rev(Percent)~., 
             labeller =  as_labeller(
  ...
  labs(
    ...
    #fill = "Planting schema",
    x = "Number of sampling units (regenerants)",
    y = "Species diversity (Hill Index, q=1)"
    ) +
  ggthemes::theme_clean()+
  scale_x_continuous(expand = c(0.001, 0), breaks = seq(0, 3500, 500)) +
  scale_y_continuous(expand = c(0, 0))+
...
fig_recruit_Hill1
ggsave("fig_Hill1_recruit.png", plot=fig_recruit_Hill1, width=90, height = 210 ,unit="mm")
```

For Hill q = 2

```{r, fig.height=8.26, fig.width=3.54}
curvas_df2 <-curvas_df|> dplyr::filter(Order.q==2)|>
  dplyr::mutate(
    Percent = ifelse(Percent=="Planted", "100", Percent),
    Percent = ordered(Percent, levels = c("25", "50", "100")),
    Planted = ifelse(Planted=="Inside", "Planted", "Unplanted"),
...
fig_recruit_Hill2<-curvas_df2 |>
  ggplot(aes(x = m, y = qD, color = Treatment, linetype = Planted)) +
  geom_line(linewidth = 1) +
  geom_ribbon(aes(ymin = qD.LCL, ymax = qD.UCL, fill = Treatment), alpha = 0.2, colour = NA) +
   geom_point(
    data = dplyr::filter(curvas_df2, Method == "Observed"),
    size = 2.5
  ) +
  facet_grid(forcats::fct_rev(Percent)~., 
             labeller =  as_labeller(
  ...
  labs(
    ...
    #fill = "Planting schema",
    x = "Number of sampling units (regenerants)",
    y = "Species diversity (Hill Index, q=2)"
    ) +
  ggthemes::theme_clean()+
  scale_x_continuous(expand = c(0.001, 0), breaks = seq(0, 3500, 500)) +
  scale_y_continuous(expand = c(0, 0))+
...
fig_recruit_Hill2
ggsave("fig_Hill2_recruit.png", plot=fig_recruit_Hill2, width=90, height = 210 ,unit="mm")
```
...
AGB_outside <- trees_clean |>
  ...
  dplyr::summarise(AGB_sum = sum(AGB_ferez, na.rm=T)/1000, .groups = "drop") |>
  dplyr::mutate(
    ratamento = ordered(tratamento, levels = ordem),
    Ano = ano-2021,
    ANO = ordered(ano),
    scaled_AGB = scale(AGB_sum)) 
```
